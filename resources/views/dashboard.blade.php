...
conn.onmessage = function(e){
	...
	if(data.image_link)
	{
		...
		document.getElementById('message_area').innerHTML = `<img src="/assets/images/`+data.image_link+`" class="img-thumbnail img-fluid" />`;
	}
	...
	if(data.response_load_unconnected_user || data.response_search_user)
	{
		...
		if(data.data.length > 0)
		{
			...
			for(var count = 0; count < data.data.length; count++)
			{
				...
				{
					user_image = `<img src="/assets/images/`+data.data[count].user_image+`" width="40" class="rounded-circle" />`;
				}
				else
				{
					user_image = `<img src="/assets/images/no-image.jpg" width="40" class="rounded-circle" />`
				}
	...
	if(data.response_connected_chat_user)
	{
		...
		if(data.data.length > 0)
		{
			for(var count = 0; count < data.data.length; count++)
			{
				...
				if(data.data[count].user_image != null)
				{
					user_image = `<img src="/assets/images/`+data.data[count].user_image+`" width="35" class="rounded-circle" />`;
				}
				else
				{
					user_image = `<img src="/assets/images/no-image.jpg" width="35" class="rounded-circle" />`;
				}
